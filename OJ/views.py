...
def home(request):
    data = requests.get('https://oj-codealong.herokuapp.com//api/v1/problems?format=json').json()
    return render(request,'home.html',{'data':data})
@login_required(login_url='login')
def problem_view(request,id):
    data = requests.get(f'https://oj-codealong.herokuapp.com//api/v1/problems/{id}?format=json').json()
